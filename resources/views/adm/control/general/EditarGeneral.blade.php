@extends('adm.cuerpo')

@section('titulo', 'Editar Producto')

@section('contenido')

<main>
	<div class="container">
	    @if(count($errors) > 0)
		<div class="col s12 card-panel red lighten-4 red-text text-darken-4">
	  		<ul>
	  			@foreach($errors->all() as $error)
	  				<li>{!!$error!!}</li>
	  			@endforeach
	  		</ul>
	  	</div>
		@endif
		@if(session('success'))
		<div class="col s12 card-panel green lighten-4 green-text text-darken-4">
			{{ session('success') }}
		</div>
		@endif

		<div class="row">
			<div class="col s12">
			{!!Form::model($general, ['route'=>['productos.update',$general->id], 'method'=>'PUT', 'files' => true])!!}	
				<div class="row">
					<div class="row">
						<img src="{{asset($general->imagen_detacada)}}" alt="">
						<div class="file-field input-field col s12">
							<div class="btn">
							    <span>Imagen</span>
							    {!! Form::file('imagen_destacada') !!}
							</div>
							<div class="file-path-wrapper">
							    {!! Form::text('imagen_destacada',null, ['class'=>'file-path validate']) !!}
							</div>
						</div>
					</div>
					<div class="input-field col s6">
						{!!Form::label('Titulo:')!!}
						{!!Form::text('titulo', $general->titulo, ['class'=>'validate', 'required'])!!}
					</div>
					
					<div class="input-field col s6">
						{!!Form::label('Orden:')!!}
						{!!Form::text('orden', $general->orden , ['class'=>'validate', 'required'])!!}
					</div>
				    <label class="col s12" for="contenido">Contenido </label>
			      	<div class="input-field col s12">
						{!!Form::textarea('contenido', $general->contenido, ['class'=>'validate', 'cols'=>'74', 'rows'=>'5'])!!}
				    </div>
				    <div class="input-field col s12">
						{!!Form::textarea('tabla', $general->tabla, ['class'=>'validate', 'cols'=>'74', 'rows'=>'5'])!!}
				    </div>
				</div>
				<div class="col s12 no-padding">
					{!!Form::submit('Actualizar', ['class'=>'waves-effect waves-light btn right'])!!}
				</div>
			{!!Form::close()!!} 
			</div>
		</div>
	</div>
</main>

<script src="//cdn.ckeditor.com/4.7.3/full/ckeditor.js"></script>
<script>
	CKEDITOR.replace('contenido');
	CKEDITOR.replace('tabla');
	CKEDITOR.config.height = '200px';
	CKEDITOR.config.width = '100%';
</script>
@endsection

